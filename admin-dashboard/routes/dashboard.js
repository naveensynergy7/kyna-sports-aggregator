...
router.get("/matches", async (req, res) => {
  try {
    ...
    if (filters.date)
      filterQueryParams.push(`date=${encodeURIComponent(filters.date)}`);
    if (filters.time)
      filterQueryParams.push(`time=${encodeURIComponent(filters.time)}`);
    if (filters.gameType)
      filterQueryParams.push(
        `gameType=${encodeURIComponent(filters.gameType)}`
      );
    if (filters.status)
      filterQueryParams.push(`status=${encodeURIComponent(filters.status)}`);
    const filterQuery =
      filterQueryParams.length > 0 ? "&" + filterQueryParams.join("&") : "";
...
router.post("/matches/:id/delete", async (req, res) => {
  try {
    ...
    await pool.execute("DELETE FROM football_matches WHERE id = ?", [matchId]);
